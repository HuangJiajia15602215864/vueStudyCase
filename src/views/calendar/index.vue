<template>
    <div class='cai-calendar-wrapper' id='cai-calendar-wrapper' style="border:1px solid red">
        <!-- 头部，即未展开的组件 -->
        <div class='cai-calendar-header'>
            <div class='cai-calendar-header-time'>
                {{currentTime.hour}}:{{formatDate(currentTime.minute)}}:{{formatDate(currentTime.second)}}</div>
        ...
        <transition name='calendar'>
            <div class='cai-calendar-container' v-if='showCalendar'>
                ...
                <div class='container-header'>
                    ...
                    <span class='cai-icon-up' @click='handlePrevMonth'>-</span>
                    <span class='cai-icon-down' @click='handleNextMonth'>+</span>
                ...
                <!-- 列表 排列每一个日期 -->
                <div class="calendar-week">
                    <div v-for="(item, index) in calendarList" :key="index" class="week-item date-item"
                        :class='[{today:isCurrentDay(item.date)}]' @click='chooseDate(item)'>
                        <span>{{item.day}}</span>
                    </div>
                </div>
            </div>
    ...
    export default {
        ...
        data() {
            return {
                ...
                interval: '', // 存储定时器
                showCalendar: '', // 是否显式完整日历
                showCalendarDialog: '', // 是否显示自定义弹窗
            }
        },
        mounted() {
            // 拖拽
        ...
        methods: {
                ...
                drag.onmousedown = function (event) {
                    ...
                    document.onmouseup = function (event) {
                        ...
                        if (startX === endX && startY === endY) {
                            if (targetClass === 'cai-calendar-header' || targetClass ===
                                'cai-calendar-header-time' || targetClass === 'cai-calendar-header-date')
                                that.turnCalendar()
                        }
            ...
            visibleCalendar(YEAR, MONTH) {
                ...
                let {
                    ...
                    month,
                    day
                } = utils.getNewDate(utils.getDate(YEAR, MONTH, 1))
